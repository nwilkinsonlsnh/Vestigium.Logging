...
public sealed class ComplexityGapTests
{
    ...
    public void CatalogAllAndBadShards()
    {
        ...
        var emptyKind = VestigiumEventCatalog.LoadFromDirectory(root, allowCustom: true);
        Assert.True(emptyKind.Count >= 15);
        ...
        File.WriteAllText(Path.Combine(shards, "one.json"), """{\"EventId\":5000,\"FullName\":\"App.One\",\"EventName\":\"One\",\"Category\":\"Network\",\"Subcategory\":\"ICMP\"}""");
        var one = VestigiumEventCatalog.LoadFromDirectory(root, allowCustom: true);
        Assert.True(one.TryGetById(5000, out _));
    }
    ...
    public void WriterRollsWhenFileExceedsLimit()
    {
        ...
        var existing = Path.Combine(dir, $"vestigium-PingIQ-{DateTime.UtcNow:yyyyMMdd}.json");
        File.WriteAllText(existing, new string('a', 200) + "\n");
        ...
        var files = Directory.GetFiles(dir, "vestigium-PingIQ-*.json");
