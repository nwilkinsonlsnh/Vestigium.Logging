...
internal sealed partial class VestigiumJsonlWriter: IVestigiumJsonlWriter
{
    ...
    private int _issued;
    private int _ioFaults;
    private int _stopped;
    private int _flushGen;
    private int _flushDone;
    ...
    public VestigiumJsonlWriter(
        string directory,
        string appId,
        long fileSizeLimitBytes = 20L * 1024 * 1024,
        TimeSpan? retainedFileTimeLimit = null,
        int retainedFileCountLimit = 90,
        ...
        Func<DateTime>? utcClock = null)
    {
        ...
        _retainTime = retainedFileTimeLimit is null || retainedFileTimeLimit <= TimeSpan.Zero
            ? TimeSpan.FromDays(14)
            : retainedFileTimeLimit.Value;
    ...
    public bool Flush(TimeSpan timeout)
    {
        ...
        if (timeout <= TimeSpan.Zero)
            return Volatile.Read(ref _written) >= targetIssued && Volatile.Read(ref _flushDone) >= gen;

        var deadline = DateTime.UtcNow + timeout;
        while (DateTime.UtcNow < deadline)
        {
            if (Volatile.Read(ref _written) >= targetIssued && Volatile.Read(ref _flushDone) >= gen)
                return true;
    ...
    public void Complete()
    {
        ...
        {
            // Writer is exiting or faulted; still close the handle.
        }
    ...
    private void Pump()
    {
        while (true)
        {
            ...
            if (gen > Volatile.Read(ref _flushDone))
            {
                ...
                {
                    Interlocked.Increment(ref _ioFaults);
                    CloseStream();
                }
    ...
    private void EnsureStream(int incomingBytes)
    {
        ...
        var needSizeRoll = _writer is not null
            && _fileLength > 0
            && _fileLength + incomingBytes > _sizeLimit;

        if (!needNewDate && !needSizeRoll && _writer is not null)
            return;
        ...
        while (true)
        {
            ...
            if (exists && existingLength > 0 && existingLength + incomingBytes > _sizeLimit)
            {
                _filePart++;
                continue;
            }
    ...
    private string FilePath(DateTime utcDate, int part)
    {
        ...
        var name = part <= 1
            ? $"vestigium-{_appId}-{stamp}.json"
            : $"vestigium-{_appId}-{stamp}-{part}.json";
    ...
    private void ApplyRetention()
    {
        try
        {
            ...
            foreach (var file in files.Where(f => f.LastWriteTimeUtc < cutoff).ToArray())
            {
                TryDelete(file);
                files.Remove(file);
            }

            while (files.Count > _retainCount)
            {
                TryDelete(files[0]);
                files.RemoveAt(0);
            }
    ...
    private void CloseStream()
    {
        ...
        try { _stream?.Flush(flushToDisk: true); } catch { /* ignore */ }
