...
Develop a robust, subscribable, centralized logging module named **Vestigium.Logging**, optimized for PowerBI ingestion. Flat JSON Lines is the only on-disk format. The engine owns schema, flood, taxonomy, subscribers, and the JSONL file writer. Asynchronous queueing and resource-aware safeguards apply across PingIQ, TraceIQ, DnsIQ, HttpIQ, and any later Vestigium module that registers an APPID.
...
| File name pattern | `vestigium-{APPID}-yyyyMMdd.json` (same-day size roll: `-n`) |
...
Every public log call returns after enqueue. Disk I/O runs on `VestigiumJsonlWriter` (`DiskQueueCapacity` 10,000, drop-oldest). Subscriber fan-out uses a bounded channel of 10,000 with `FullMode = DropOldest`. UI drain: `VestigiumLogPump` every 100 ms or 50 events, whichever first.
...
| Method | Behavior |
|---|---|
| `Flush()` / `Flush(timeout)` | Drain flood summaries and wait up to `FlushTimeout` (default 5 seconds) for the disk queue. **Writes keep being accepted.** |
| `Shutdown()` | Stop accepting writes, wait for in-flight `Emit` calls, drain, wait for the disk queue, close the file handle, complete subscribers. |
...
The catalog is frozen at the end of `Initialize`. Later `Register` throws. `EventReader` is a single-consumer channel; UI hosts use one `VestigiumLogPump`. Variable fields belong in `PROPERTIES`, not MESSAGE. At 90% of `FloodIdentityCap` the engine emits one internal Warning.
...
6. Normal process exit calls `Shutdown`, which drains the disk queue and closes the file handle.
...
|---|---|---|
| 1.0 | Gemini-revised requirements | Gemini thread |
...
| 1.8 | P4: in-flight shutdown drain, frozen taxonomy, flood near-cap warning | Review follow-up, 17 Sep 2026 |
| 1.9 | S0 owned JSONL writer: DiskQueueCapacity | VEST-LOG-IMP-002, 17 Sep 2026 |
| 1.10 | S2/S3: owned JSONL writer shipped; Serilog packages removed | VEST-LOG-IMP-002, 17 Sep 2026 |
